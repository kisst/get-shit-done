...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" init plan-phase "${PHASE}")
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" history-digest
```
...
```bash
VALID=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" verify plan-structure "$PLAN_PATH")
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
