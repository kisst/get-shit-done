...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" verify plan-structure "$PLAN_PATH")
```
