...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" websearch "your query" --limit 10
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
