...
```bash
INIT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" init phase-op "${target}")
```
...
```bash
RESULT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" phase remove "${target}" --force)
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
