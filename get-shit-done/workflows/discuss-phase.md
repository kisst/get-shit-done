...
```bash
INIT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" init phase-op "${PHASE}")
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
