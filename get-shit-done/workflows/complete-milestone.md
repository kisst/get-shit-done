...
```bash
ROADMAP=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" roadmap analyze)
```
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**Commit:**
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" init execute-phase "1")
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
