...
```bash
TRANSITION=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" progress bar --raw)
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" config-set workflow.auto_advance false
```
