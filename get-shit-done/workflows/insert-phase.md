...
```bash
INIT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" init phase-op "${after_phase}")
```
...
```bash
RESULT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" phase insert "${after_phase}" "${description}")
```
