...
```bash
timestamp=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
