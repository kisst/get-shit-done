...
```bash
INIT=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" init quick "$DESCRIPTION")
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
