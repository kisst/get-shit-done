...
```bash
PHASE_INFO=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```bash
python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" commit-docs "docs(phase-${PHASE}): add validation strategy"
```
