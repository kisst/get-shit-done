...
```bash
PHASE_INFO=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(python3 "$HOME/.claude/get-shit-done/bin/gsd-tools.py" find-phase "${PHASE}" --raw)
```
